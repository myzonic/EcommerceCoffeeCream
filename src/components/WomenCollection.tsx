...
import { Venus, CheckCircle, ShoppingBag } from 'lucide-react';
  ...
  const renderCard = (item: Product) => {
    ...
    return (
      <motion.div
        ...
        className="flex flex-col justify-between p-6 sm:p-8 bg-brand-pearl rounded-[40px] shadow-sm border border-brand-gold/20 relative group overflow-hidden cursor-pointer transition-shadow hover:shadow-xl"
      >
          ...
          >
            {isAdded ? (
              <>
                ...
                <span>Added To Your Loadout</span>
              </>
            ) : (
              <>
  ...
  return (
    <section id="womens-collection" className="relative py-20 px-4 md:px-8 bg-brand-cream overflow-hidden scroll-mt-24">
      ...
      <div className="max-w-7xl mx-auto">
        ...
        >
          {standardProducts.map(renderCard)}
        </motion.div>
